...
impl BsonCoder {
    ...
    pub(crate) fn decode<'a>(namespace: &Namespace, model: &Model, r#type: &Type, optional: bool, bson_value: &Bson, path: impl AsRef<KeyPath>) -> Result<Value> {
        ...
        match r#type {
            ...
            Type::EnumVariant(path, string_path) => match bson_value.as_str() {
                Some(val) => {
                    ...
                    if e.cache.member_names.contains_str(val) {
                        ...
                        Err(Error::record_decoding_error(model.name(), path, format!("string value for enum `{}'", e.path.join("."))))
                    }
                },
                None => Err(Error::record_decoding_error(model.name(), path, "string")),
            }
            Type::Array(inner_field) => {
                match bson_value.as_array() {
                    Some(arr) => Ok(Value::Array(arr.iter().enumerate().map(|(i, v)| {
                        let path = path + i;
                        Self::decode(namespace, model, inner_field.r#type(), inner_field.is_optional(), v, path)
            ...
            Type::Dictionary(inner_field) => {
                match bson_value.as_document() {
                    Some(doc) => Ok(Value::Dictionary(doc.iter().map(|(k, v)| {
                        let path = path + k;
                        Ok((k.to_owned(), Self::decode(namespace, model, inner_field.r#type(), inner_field.is_optional(), v, path)?))
